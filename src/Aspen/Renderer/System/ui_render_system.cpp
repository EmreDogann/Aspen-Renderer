...
namespace Aspen {
	...
	UIRenderSystem::UIRenderSystem(Device& device, Renderer& renderer)
		...
		// pipeline = std::make_shared<Pipeline>(device, "assets/shaders/simple_shader.vert.spv", "assets/shaders/simple_shader.frag.spv");
		// createPipelines();
	}
	...
	void UIRenderSystem::renderGameObjects(FrameInfo& frameInfo, std::shared_ptr<Scene>& scene) { // Flush changes to update on the GPU side.
